...
impl LevelSelectionAccessor for LdtkProject<ExternalLevelMetadata> {
    fn get_indices_for_iid(&self, iid: &String) -> Option<&LevelIndices> {
        Some(self.level_map().get(iid)?.metadata().indices())
    }
}

impl LdtkProject<ExternalLevelMetadata> {
